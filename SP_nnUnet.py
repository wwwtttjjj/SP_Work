...
import random
from same_function import get_data,get_args,get_current_consistency_weight,update_ema_variables,val_epoch,create_model
#定义一些保存路径
...
#The MT 
def train_epoch(phase, epoch, model, ema_model, dataloader,optimizer):
...
def main(args, device):
    ...
    best_value = 0
    for epoch in epochs:
        info["train"] = train_epoch("train", epoch, model=model,ema_model=ema_model, dataloader=dataloaders["train"],optimizer=optimizer)
